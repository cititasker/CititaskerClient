...
export default function TaskDetailsClient({ taskId }: TaskDetailsClientProps) {
  ...
  const { data, isLoading } = useFetchUserTaskById({ id: taskId });
  ...
  }, [task, questions, actions.openOfferModal]);
  ...
  return (
    <FormProvider {...actions.methods}>
      <div className="min-h-screen bg-white sm:bg-background-secondary overflow-x-hidden">
        <div className="container-w p-top pb-5 lg:h-screen lg:flex lg:flex-col lg:overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 lg:flex-1 lg:min-h-0">
            ...
            <div className="lg:col-span-2 space-y-5 lg:overflow-y-auto no-scrollbar">
              <TaskStatusCard
                date={task.date}
            ...
            <div className="lg:col-span-3 lg:min-h-0 lg:overflow-hidden w-full min-w-0">
              <CustomTab
                items={tabs}
                className="h-fit lg:flex lg:flex-col"
                contentClassName="sm:p-5 md:p-8 h-fit bg-transparent sm:bg-white"
                mobileAsCards={false}
              />
            </div>
          </div>
