"use client";
import React from "react";
import FormButton from "@/components/forms/FormButton";
import { cn } from "@/utils";
import { ROUTES } from "@/constant";

interface IProps {
  extraClass?: string;
  children: React.ReactNode;
  data: {
    title: string;
    list: string[];
  };
}
const WhyCitiTaskerItem = ({
  children,
  extraClass,
  data: { title, list },
}: IProps) => {
  return (
    <div
      className={cn(
        "tasker_section gap-y-20 flex flex-col-reverse md:flex-row items-stretch md:even:flex-row-reverse  gap-x-[3.5rem]",
        extraClass
      )}
    >
      <div className="flex-1 h-fit">{children}</div>
      <div className="bg-white py-2.5 z-10 h-fit hidden md:inline-block">
        <span className="w-[1.875rem] h-[1.875rem] rounded-full bg-light-primary-1 flex justify-center items-center">
          <span className="w-5 h-5 rounded-full bg-light-primary-2 flex justify-center items-center">
            <span className="w-2.5 h-2.5 rounded-full bg-primary"></span>
          </span>
        </span>
      </div>
      <div className="flex-1">
        <div className="h-fit text-center md:text-left my-auto w-fit max-w-[450px] md:max-w-full mx-auto flex flex-col items-center md:items-start">
          <h2 className="text-[2rem] md::text-[2.5rem] text-dark-secondary font-bold mb-3 md:mb-5">
            {title}
          </h2>
          <ul className="mb-10">
            {list.map((el, i) => (
              <li key={i} className="flex items-start">
                <p className="text-dark-secondary text-base font-normal">
                  {el}
                </p>
              </li>
            ))}
          </ul>
          <FormButton
            variant="outline"
            text="Earn more as a Tasker"
            href={ROUTES.TASKER}
            className="border-primary text-primary"
          />
        </div>
      </div>
    </div>
  );
};

export default WhyCitiTaskerItem;
